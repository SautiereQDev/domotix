...
class TestDeviceCreateCommandsIntegration:
    ...
    def test_create_light_with_persistence(self):
        ...
        with patch("domotix.cli.device_cmds.ControllerFactory") as mock_factory:
            # Mock du contrôleur
...
class TestCLIPersistenceErrorHandling:
    ...
    def test_create_light_failure(self):
        ...
        with patch("domotix.cli.device_cmds.ControllerFactory") as mock_factory:
            # Mock du contrôleur qui échoue
            mock_controller = Mock()
            mock_controller.create_light.return_value = None
            mock_factory.create_light_controller.return_value = mock_controller
            ...
            with patch("domotix.cli.device_cmds.create_session") as mock_session:
                ...
                with patch("builtins.print") as mock_print:
                    ...
                    error_printed = any(
                        "erreur" in str(call).lower() or "échec" in str(call).lower()
    ...
    def test_device_not_found(self):
        ...
        with patch("domotix.cli.device_cmds.ControllerFactory") as mock_factory:
            # Mock du contrôleur qui ne trouve pas le dispositif
            mock_controller = Mock()
            ...
            with patch("domotix.cli.device_cmds.create_session") as mock_session:
                ...
                with patch("builtins.print") as mock_print:
                    ...
                    # Vérifier qu'il y a un message d'erreur
                    error_printed = any(
                        "introuvable" in str(call).lower()
                        or "pas trouvé" in str(call).lower()
                        for call in mock_print.call_args_list
                    )
                    assert error_printed
    ...
    def test_operation_failure(self):
        ...
        with patch("domotix.cli.device_cmds.ControllerFactory") as mock_factory:
            # Mock du contrôleur qui échoue l'opération
            mock_controller = Mock()
            ...
            with patch("domotix.cli.device_cmds.create_session") as mock_session:
                ...
                with patch("builtins.print") as mock_print:
                    ...
                    # Vérifier qu'il y a un message d'erreur
                    error_printed = any(
                        "échec" in str(call).lower() or "erreur" in str(call).lower()
                        for call in mock_print.call_args_list
                    )
                    assert error_printed
...
class TestCLIRealDatabaseIntegration:
    ...
    def test_full_lifecycle_with_real_db(self, temp_db):
        ...
        # Patch la configuration pour utiliser notre DB temporaire
        with patch("domotix.core.database.DATABASE_URL", f"sqlite:///{temp_db}"):
            # Créer une lampe
